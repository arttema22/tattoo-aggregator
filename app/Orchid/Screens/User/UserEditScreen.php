...
class UserEditScreen extends Screen
{
    ...
    public function commandBar(): iterable
    {
        return [
            ...
            Button::make(__('Верифицировать'))
                ->icon('check')
                ->confirm('Данным действием, вы подтверждаете что этот аккаунт принадлежит указанной почте. Теперь обладатель аккаунта сможет входить в свой личный кабинет.')
                ->method('verify')
                ->canSee( $this->user->exists && ( $this->user?->role ?? 0 ) === 3 && ( $this->user?->profile->approved ?? 0 ) === 0 ),
    ...
    public function verify(User $user) : RedirectResponse
    {
        ...
        if ( $user->role === 3 && $user->profile->approved === 0 ) {
